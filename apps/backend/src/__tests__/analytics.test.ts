...
describe('Analytics API', () => {
  ...
  describe('GET /api/analytics/overview', () => {
    it('200 — returns analytics overview', async () => {
      prismaMock.cardView.count
        .mockResolvedValueOnce(100)
        .mockResolvedValueOnce(10);

      prismaMock.followLog.count.mockResolvedValue(5);
      ...
      expect(body.viewsToday).toBe(10);
  ...
  describe('GET /api/analytics/views', () => {
    it('200 — returns paginated views', async () => {
      prismaMock.cardView.count.mockResolvedValue(45);
      ...
      const res = await app.inject({
        method: 'GET',
        url: '/api/analytics/views?page=2',
        headers: authHeader(),
      });
    ...
    it('200 — filters by cardId when provided', async () => {
      prismaMock.cardView.count.mockResolvedValue(0);
      prismaMock.cardView.findMany.mockResolvedValue([]);

      const res = await app.inject({
        method: 'GET',
        url: '/api/analytics/views?cardId=card-123',
        headers: authHeader(),
      });
    ...
    it('200 — defaults to page 1', async () => {
      prismaMock.cardView.count.mockResolvedValue(0);
      prismaMock.cardView.findMany.mockResolvedValue([]);
