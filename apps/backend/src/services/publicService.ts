...
export async function getPublicProfile(app: FastifyInstance, username: string, viewerId: string | null, request: any) {
    ...
    try {
      ...
      if (cached) {
        ...
        if (viewerId && viewerId !== _userId) {
          ...
          dispatchWebhook(app.prisma, _userId, 'card.viewed', { event: 'card.viewed', cardId: null, viewerId, source: request.query?.source || 'link', timestamp: new Date().toISOString() }).catch((err: unknown) => app.log.error(`Webhook dispatch failed: ${getErrorMessage(err)}`))
        }
  ...
  if (viewerId && viewerId !== user.id) {
    ...
    dispatchWebhook(app.prisma, user.id, 'card.viewed', { event: 'card.viewed', cardId: null, viewerId, source: request.query?.source || 'link', timestamp: new Date().toISOString() }).catch((error: unknown) => app.log.error(`Webhook dispatch failed: ${getErrorMessage(error)}`))
  }
...
export async function getUserCard(app: FastifyInstance, username: string, cardId: string, viewerId: string | null, request: any) {
  ...
  if (viewerId && viewerId !== user.id) {
    ...
    dispatchWebhook(app.prisma, user.id, 'card.viewed', { event: 'card.viewed', cardId: card.id, viewerId, source: request.query?.source || 'qr', timestamp: new Date().toISOString() }).catch((error: unknown) => app.log.error(`Webhook dispatch failed: ${getErrorMessage(error)}`))
  }
