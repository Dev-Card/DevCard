<script lang="ts">
  ...
  const BACKEND_URL = import.meta.env.PUBLIC_API_URL || (typeof process !== 'undefined' && process.env?.BACKEND_URL) || 'http://localhost:3000';

  onMount(() => {
    isMounted = true;
    // Check for error query param from backend
  ...
  function handleOAuth(provider: 'github' | 'google') {
    ...
    // Check if backend is likely up (optional but helpful)
    setTimeout(() => {
...
<main class="min-h-screen flex items-center justify-center p-6 bg-grid relative overflow-hidden">
  ...
  <div class="absolute top-0 left-0 w-full h-full pointer-events-none overflow-hidden -z-10">
    <div class="absolute top-1/4 -left-20 w-[500px] h-[500px] bg-primary/10 rounded-full blur-[120px] animate-pulse-slow"></div>
    <div class="absolute bottom-1/4 -right-20 w-[500px] h-[500px] bg-secondary/10 rounded-full blur-[120px] animate-pulse-slow" style="animation-delay: 2s"></div>
  </div>
  ...
  {#if isMounted}
    ...
    >
      <div class="text-center mb-10">
        ...
        <h1 class="text-3xl font-black mb-3">Welcome Back</h1>
        <p class="text-(--text-muted) font-medium">Continue your journey in the developer ecosystem.</p>
      </div>
      ...
      <div class="glass rounded-[2.5rem] p-10 shadow-2xl border border-white/10 relative overflow-hidden">
        ...
        <div class="my-8 flex items-center gap-4">
          ...
          <span class="text-[10px] font-black uppercase tracking-[0.2em] text-(--text-muted)">Security Notice</span>
          <div class="h-px flex-1 bg-(--border-main)"></div>
        </div>
      ...
      <p class="mt-8 text-center text-sm font-bold text-(--text-muted)">
        ...
        <a href="/signup" class="text-primary hover:underline ml-1">Create one for free</a>
      </p>
    </div>
  {/if}
</main>
